    ...
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div>
        <div class="flex flex-wrap items-center gap-2">
          <span class="rounded-full px-2 py-1 text-xs font-semibold ring-1" :class="severityClass">{{ incident.severity }}</span>
        ...
        <RouterLink class="mt-3 block text-base font-semibold text-zinc-50 hover:text-cyan-200" :to="`/incidents/${incident.id}`">
          {{ incident.title }}
        </RouterLink>
        <p class="mt-1 text-sm text-zinc-400">{{ incident.alertCount }} alert{{ incident.alertCount === 1 ? '' : 's' }}</p>
      </div>
      <div v-if="!authStore.isViewer" class="flex shrink-0 gap-2">
        <button
