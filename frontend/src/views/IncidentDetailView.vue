      ...
      <template v-if="incident">
        ...
        <section class="grid gap-4 md:grid-cols-4">
          ...
          <div class="rounded-md border border-zinc-800 p-4">
            ...
            <p class="mt-2 text-sm">
              {{ incident.acknowledgedAt ? new Date(incident.acknowledgedAt).toLocaleString() : 'Pending' }}
            </p>
          </div>
          <div class="rounded-md border border-zinc-800 p-4">
            <p class="text-xs text-zinc-500">
            ...
            <p class="mt-2 text-sm">
              {{ incident.status === 'INVESTIGATING' ? 'Active' : 'Not active' }}
            </p>
          </div>
          <div class="rounded-md border border-zinc-800 p-4">
            <p class="text-xs text-zinc-500">
            ...
            <p class="mt-2 text-sm">
              {{ incident.resolvedAt ? new Date(incident.resolvedAt).toLocaleString() : 'Pending' }}
            </p>
          </div>
        </section>
        ...
        >
          ...
          <textarea
            ...
            class="mt-2 min-h-24 w-full rounded-md border border-zinc-800 bg-zinc-950 p-3 text-sm"
          />
        ...
        <section class="rounded-md border border-zinc-800 p-4">
          ...
          <div class="mt-3 grid gap-2">
            <article
              ...
              :key="alert.id"
              class="rounded-md bg-zinc-900 p-3 text-sm"
            >
              <p class="font-medium">
                {{ alert.source }} / {{ alert.alertName }}
              </p>
            ...
            >
              No alert payloads are attached yet.
            </p>
          </div>
        </section>

        <section
          v-if="incident.runbook"
          class="rounded-md border border-zinc-800 p-4"
        >
          <h2 class="text-sm font-semibold">
        ...
        >
          ...
          <input
            v-model="postmortemSummary"
            class="mt-3 w-full rounded-md border border-zinc-800 bg-zinc-950 p-3 text-sm"
            placeholder="Summary"
          >
          <textarea
            v-model="postmortemTimeline"
            class="mt-3 min-h-24 w-full rounded-md border border-zinc-800 bg-zinc-950 p-3 text-sm"
            placeholder="Timeline"
          />
          <textarea
            v-model="postmortemActionItems"
            class="mt-3 min-h-24 w-full rounded-md border border-zinc-800 bg-zinc-950 p-3 text-sm"
            placeholder="Action items, one per line"
          />
          <button
            class="mt-3 rounded-md bg-cyan-400 px-3 py-2 text-sm font-semibold text-zinc-950"
            type="button"
            @click="createPostmortem"
          >
            Save postmortem
          </button>
        </section>
