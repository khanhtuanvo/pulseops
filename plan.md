...
| 15.4 | Harden error handling across all resolvers | [x] |
...
5. ~~**15.4 Harden error handling across resolvers**~~ — ✅ **Done** (this session). Added `internal/graph/errors.go`: `mapError` classifies into `UNAUTHENTICATED`/`FORBIDDEN`/`BAD_USER_INPUT`/`INTERNAL_ERROR`, and a central gqlgen `ErrorPresenter` (wired in `router.go`) sanitises every resolver error — unknown/Mongo errors collapse to a generic `INTERNAL_ERROR` (logged server-side) while gqlgen-native parse/validation errors pass through. Added a `RecoverFunc` so resolver panics return a clean error. Added input validation (`requireNonEmpty`, `validateTimeRange`, `validateRotation`) to the create/update mutations and time-range queries. Unit tests in `errors_test.go` assert no internal text leaks. *Auth + teamId scoping were audited and already correct in every resolver* (each starts with `RequireAuth`/`requireOwner`; all repo calls pass `claims.TeamID`). golangci-lint not run locally (not installed) — `go vet`/`gofmt` clean; CI runs the linter.
